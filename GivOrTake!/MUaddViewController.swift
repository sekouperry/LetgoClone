...
class MUaddViewController: UIViewController,UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func savePost(_ sender: CustomizableButtons) {
        ...
        if postSwitch.isOn {
                ...
                self.netService.uploadImageToFirebase(postId: postId, imageData: imageData, completion: { (imgUrl) in
                    ...
                    if let url = self.videoURL {
                        ...
                        self.netService.uploadVideoToFirebase(postId: postId, videoURL: url, completion: { (vUrl) in
                            ...
                            let post = Posts(postId: postId, userId: FIRAuth.auth()!.currentUser!.uid, postText: postText, postImageURL: String(describing:imgUrl), postVideoURL: String(describing:vUrl), postDate: postDate, type:"VIDEO")
                            
                            self.netService.MUsavePostToDB(post: post, completed: {
                                SwiftLoader.hide()
                                self.dismiss(animated: true, completion: nil)
                            })
                        ...
                        let post = Posts(postId: postId, userId: FIRAuth.auth()!.currentUser!.uid, postText: postText, postImageURL: String(describing:imgUrl), postDate: postDate, type:"IMAGE")
            ...
            let post = Posts(postId: postId, userId: FIRAuth.auth()!.currentUser!.uid, postText: postText, postImageURL: "", postDate: postDate, type: "TEXT")
