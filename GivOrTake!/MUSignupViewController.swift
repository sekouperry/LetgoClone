...
class MUSignupViewController: UIViewController, UITextFieldDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, UIPopoverPresentationControllerDelegate  {
    ...
    @IBOutlet weak var reenterPasswordTextField: MUCustomizableTextfields! {
        didSet{
            reenterPasswordTextField.delegate = self
        }
    }
    
    @IBOutlet weak var firstnameTextField: MUCustomizableTextfields!{
        didSet{
            firstnameTextField.delegate = self
        }
    }
    
    @IBOutlet weak var lastnameTextField: MUCustomizableTextfields! {
        didSet{
    ...
    @IBOutlet weak var createAccountButton: MUCustomizableButtons!
    ...
    @IBAction func createAccountAction(_ sender: MUCustomizableButtons) {
        ...
        if finalEmail.isEmpty || password.isEmpty || reenterPassword.isEmpty || firstname.isEmpty || lastname.isEmpty {
            ...
            if password == reenterPassword {
                ...
                if isValidEmail(email: finalEmail) {
                    self.networkingService.signUp(firstname: firstname, lastname: lastname, email: finalEmail, pictureData: data, password: password, type: 1)
                }
...
extension MUSignupViewController {
    ...
    func setTapGestureRecognizerOnView(){
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(MUSignupViewController.hideKeyboardOnTap))
    ...
    func setSwipeGestureRecognizerOnView(){
        let swipDown = UISwipeGestureRecognizer(target: self, action: #selector(MUSignupViewController.hideKeyboardOnTap))
